(function(angular) {
  var InfoFactory =  function($resource) {
      return $resource('/info', {}, {
              get: {
                  method: 'GET',
                  isArray: false,
                  headers: { 
                     'cid': calcCorrelationId
                  }
              }
          });
    }

    var AdminFactory =  function($http) {
      console.log('AdminFactory build');
      return {
         'changeBackend' : function(newName, callBack) {
            console.log("Admin.changeBackend("+newName+")");
            $http({
                method: 'POST',
                url: '/lc/backend',
                data: {
                    'newBackend': newName
                }
            }).then(function successCallback(response) {
                console.log("OK: "+response.status+"  "+JSON.stringify(response.data));
                callBack();
            }, function errorCallback(response) {
                console.log("failed: "+response.status+"  "+JSON.stringify(response.data));
            });
          
         }
            
      };
    }


    function calcCorrelationId(){
        // see 
        // http://www.ietf.org/rfc/rfc4122.txt
        var s = [];
        var hexDigits = "0123456789abcdef";
        for (var i = 0; i < 36; i++) {
            s[i] = hexDigits.substr(Math.floor(Math.random() * 0x10), 1);
        }
        s[14] = "4";  // bits 12-15 of the time_hi_and_version field to 0010
        s[19] = hexDigits.substr((s[19] & 0x3) | 0x8, 1);  // bits 6-7 of the clock_seq_hi_and_reserved to 01
        s[8] = s[13] = s[18] = s[23] = "-";
    
        var uuid = s.join("");
        return uuid;
    }

  
    InfoFactory.$inject = ['$resource'];
    AdminFactory.$inject = ['$http'];
    myMod = angular.module("myApp.services");
    myMod.factory("Info", InfoFactory);
    myMod.factory("Admin", AdminFactory);
 }(angular));




